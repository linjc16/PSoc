...
#if defined(CYDEV_BOOTLOADER_IO_COMP) && (0u != ((CYDEV_BOOTLOADER_IO_COMP == CyBtldr_UART_Blue) || \
                                          (CYDEV_BOOTLOADER_IO_COMP == CyBtldr_Custom_Interface)))


/*******************************************************************************
* Function Name: UART_Blue_CyBtldrCommStart
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: UART_Blue_CyBtldrCommStop
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: UART_Blue_CyBtldrCommReset
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: UART_Blue_CyBtldrCommWrite
********************************************************************************
*
* Summary:
         ...
         
{
    ...
    while(bufIndex < size)
    {
        UART_Blue_PutChar(pData[bufIndex]);
        bufIndex++;
    }
...
/*******************************************************************************
* Function Name: UART_Blue_CyBtldrCommRead
********************************************************************************
*
* Summary:
         ...
         
{
    ...
    *  If at least one byte is received within the time out interval, wait for more data */
    for (iCntr = 0u; iCntr < ((uint16)10u * timeOut); iCntr++)
    {
        ...
        */
        if(0u != UART_Blue_GetRxBufferSize())
        {
            ...
            */
            do
            {
                oldDataCount = UART_Blue_GetRxBufferSize();
                CyDelay(UART_Blue_BYTE2BYTE_TIME_OUT);
            }
            while(UART_Blue_GetRxBufferSize() > oldDataCount);

            status = CYRET_SUCCESS;
            break;
        }
        /* If the data is not received, give a delay of 
        *  UART_Blue_BL_CHK_DELAY_MS and check again until the timeOut specified.
        */
        else
        {
            CyDelay(UART_Blue_BL_CHK_DELAY_MS);
        }
    }
    ...
    *count = 0u;
    ...
    while(UART_Blue_GetRxBufferSize() > 0u)
    {
        tempCount = UART_Blue_GetRxBufferSize();
        *count  =(*count) + tempCount;
        ...
        if(*count < size)
        {
            for (iCntr = 0u; iCntr < tempCount; iCntr++)
            {
                ...
                pData[dataIndexCntr] = UART_Blue_ReadRxData();
                dataIndexCntr++;
            }

            /* Check if the last received byte is end of packet defined by bootloader
            *  If not wait for additional UART_Blue_WAIT_EOP_DELAY ms.
            */
            if(pData[dataIndexCntr - 1u] != UART_Blue_PACKET_EOP)
            {
                CyDelay(UART_Blue_WAIT_EOP_DELAY);
            }
        }
